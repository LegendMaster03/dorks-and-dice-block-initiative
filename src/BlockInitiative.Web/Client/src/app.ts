...
<header class="bi-grid">
  ...
  <div class="bi-steps">
    <div class="bi-step"><strong>1. Build the roster</strong><div class="bi-muted">Players are simple. Enemies can use tactical groups and Rules Core monster data.</div></div>
    <div class="bi-step"><strong>2. Build blocks</strong><div class="bi-muted">Placement determines blocks. Kaiju remain special blocks.</div></div>
    <div class="bi-step"><strong>3. Run combat</strong><div class="bi-muted">Track the active block, health, and Kaiju state.</div></div>
  </div>
  <details><summary>How the block rule works</summary><p class="mb-0">Initiative is sorted normally. Consecutive allies form blocks. Players in the same player block may act in any order. If the first and last blocks are allied and use the same block type, the last skips its separate round-one activation and joins the first at the start of round two.</p></details>
</header>
<section class="card card-body bi-grid" data-role="setup">
  ...
  <div class="bi-sides">
    ...
    <section class="bi-side">
      <div class="bi-side-head"><div><strong>Enemies</strong><div class="bi-muted">Choose how enemy initiative is placed, then build the roster.</div></div><button class="btn btn-sm btn-outline-secondary" data-action="add-kaiju">+ Kaiju</button></div>
      <div class="bi-side-body">
        <div class="bi-enemy-options">
          <div class="bi-field"><label>Enemy initiative method</label><select data-role="enemy-method"><option value="average">Tactical groups — average member rolls</option><option value="individual">Individual rolls — group by placement</option><option value="shared">One roll per tactical group</option></select></div>
          <div class="bi-muted" data-role="enemy-method-help"></div>
        </div>
        <div class="bi-groups" data-role="enemy-groups"></div>
        <div class="bi-row"><button class="btn btn-sm btn-outline-secondary" data-action="add-group">+ Tactical group</button><span class="bi-muted">Groups are DM-authored roster units; turn blocks are still derived from initiative placement.</span></div>
...
function addCombatant(allianceId: string, blockType: TurnBlockType = "standard", focus = true, target?: HTMLElement): HTMLElement {
    ...
    field<HTMLSelectElement>(card, "block-type").addEventListener("change", () => { const group = card.closest<HTMLElement>(".bi-tactical-group"); if (group && type(card) === "kaiju") { card.dataset.groupId = ""; kaijuList.append(card); updateGroupSummary(group); } updateBadges(card); changed(); });
    if (allianceId === "enemies" && blockType === "standard") attachMonsterAutocomplete(card);
...
function applyMonsterTemplate(card: HTMLElement, template: MonsterTemplate, cloning: boolean): void {
    ...
    const actions = query<HTMLElement>("[data-role='template-actions']", card); const clone = query<HTMLButtonElement>("[data-action='clone-monster']", card); clone.textContent = `+ Another ${template.name}`; clone.onclick = () => { const parentGroup = card.closest<HTMLElement>(".bi-tactical-group"); if (parentGroup) addEnemyToGroup(parentGroup, template, false); }; actions.hidden = !group;
...
function clonePrimaryMonster(group: HTMLElement): void {
    ...
    addEnemyToGroup(group, templates[0], false);
}

function updateEnemyMethodUi(): void {
    const mode = currentGroupMode();
    enemyMethodHelp.textContent = mode === "average" ? "Roll each enemy. Every tactical group's member rolls are averaged, and that average determines the group's initiative position." : mode === "shared" ? "Roll once for each tactical group. Every member of that group occupies the shared initiative position." : "Every enemy uses its own roll. Tactical groups remain useful for roster organization and cloning, but do not affect placement.";
...
function updateGroupModeUi(group: HTMLElement): void {
    const mode = currentGroupMode(); query<HTMLElement>("[data-role='shared-roll-wrap']", group).hidden = mode !== "shared";
...
function updateGroupSummary(group: HTMLElement): void {
    ...
    else if (mode === "shared") { const raw = query<HTMLInputElement>("[data-role='shared-roll']", group).value.trim(); placement = raw ? `group initiative ${raw}` : "group initiative not entered"; }
    else { const values = members.map(card => Number(field<HTMLInputElement>(card, "initiative").value)).filter(Number.isFinite); placement = values.length === members.length && values.length ? `average ${formatNumber(values.reduce((sum, value) => sum + value, 0) / values.length)}` : "average waiting for member rolls"; }
    summary.textContent = `${members.length} ${members.length === 1 ? "enemy" : "enemies"} · ${placement}`;
    const cloneButton = query<HTMLButtonElement>("[data-action='clone-primary']", group); const templates = members.map(card => monsterTemplates.get(card.dataset.id ?? "")).filter((value): value is MonsterTemplate => Boolean(value)); const sameTemplate = templates.length === members.length && templates.length > 0 && templates.every(template => template.match.id === templates[0].match.id); cloneButton.hidden = !sameTemplate; if (sameTemplate) cloneButton.textContent = `+ Another ${templates[0].name}`;
}
...
function setLabeledNumber(scope: HTMLElement, labelText: string, value: number): void {
    for (const wrapper of scope.querySelectorAll<HTMLElement>(".bi-field")) { const label = wrapper.querySelector("label")?.textContent?.trim(); const input = wrapper.querySelector<HTMLInputElement>("input[type='number']"); if (label === labelText && input) { input.value = String(value); input.dispatchEvent(new Event("change", { bubbles: true })); return; } }
}
...
function type(card: HTMLElement): TurnBlockType { return field<HTMLSelectElement>(card, "block-type").value === "kaiju" ? "kaiju" : "standard"; }
function tacticalGroupId(card: HTMLElement): string | null { if (card.dataset.alliance !== "enemies" || type(card) !== "standard" || currentGroupMode() === "individual") return null; return card.closest<HTMLElement>(".bi-tactical-group")?.dataset.groupId || null; }

function updateBadges(card: HTMLElement): void {
    const box = query<HTMLElement>("[data-role='badges']", card); box.replaceChildren(badge(friendly(alliance(card) || card.dataset.alliance || "Other side"))); if (type(card) === "kaiju") box.append(badge("Kaiju block", true)); const group = card.closest<HTMLElement>(".bi-tactical-group"); if (group) box.append(badge(query<HTMLInputElement>("[data-role='group-name']", group).value || "Tactical group"));
}
...
function renderPreview(preview: InitiativePreviewResponse, request: InitiativePreviewRequest): void {
    ...
    const summary = document.createElement("section"); summary.className = "card card-body"; const summaryRow = document.createElement("div"); summaryRow.className = "bi-row"; const text = document.createElement("div"); const title = document.createElement("h3"); title.className = "h5 mb-1"; title.textContent = preview.requiresAdjudication ? "One ruling is needed" : "Initiative blocks are ready"; const detail = document.createElement("div"); detail.className = "bi-muted"; detail.textContent = preview.requiresAdjudication ? "Resolve the opposing tie below before starting combat." : "Review the derived order, then start encounter tracking."; text.append(title, detail); summaryRow.append(text, badge(`${preview.blocks.length} block${preview.blocks.length === 1 ? "" : "s"}`)); summary.append(summaryRow); results.append(summary);
    if (preview.issues.length) results.append(renderTie(preview, request));
    if (preview.usesManualOrderOverride) { const note = document.createElement("div"); note.className = "bi-message bi-success"; note.textContent = "DM tie order applied. Original initiative rolls and tactical-group averages are unchanged."; results.append(note); }
    const byId = new Map(preview.orderedCombatants.map(combatant => [combatant.id, combatant])); const section = document.createElement("section"); section.className = "card card-body"; const heading = document.createElement("h3"); heading.className = "h5 mb-1"; heading.textContent = "Block order"; const description = document.createElement("p"); description.className = "bi-muted"; description.textContent = "Tactical groups determine enemy placement only when selected above. Adjacent allied placements still derive the actual turn blocks."; section.append(heading, description);
    const list = document.createElement("div"); list.className = "bi-blocks";
    preview.blocks.forEach((block, index) => { const element = document.createElement("article"); element.className = "bi-block"; const head = document.createElement("div"); head.className = "bi-block-head"; const strong = document.createElement("strong"); strong.textContent = `Block ${index + 1}`; head.append(strong, blockBadges(block.allianceId, block.blockType)); const body = document.createElement("div"); body.className = "bi-block-body"; for (const id of block.memberOrder) { const combatant = byId.get(id); const row = document.createElement("div"); row.className = "bi-member"; const name = document.createElement("span"); name.textContent = combatant?.name ?? id; const initiative = document.createElement("span"); initiative.className = "bi-muted"; if (combatant) { const groupLabel = combatant.tacticalGroupId ? ` · ${groupName(combatant.tacticalGroupId)}` : ""; const effective = combatant.effectiveInitiative !== combatant.initiativeTotal ? ` → position ${formatNumber(combatant.effectiveInitiative)}` : ""; initiative.textContent = `Roll ${formatNumber(combatant.initiativeTotal)}${effective}${groupLabel}`; } row.append(name, initiative); body.append(row); } if (block.allianceId === "players" && block.memberOrder.length > 1) { const note = document.createElement("div"); note.className = "bi-muted mt-2"; note.textContent = "Players in this block may choose their order."; body.append(note); } element.append(head, body); list.append(element); });
    section.append(list, roundBoundary(preview));
    if (!preview.requiresAdjudication) { const action = document.createElement("div"); action.className = "bi-row bi-primary"; const actionText = document.createElement("div"); actionText.innerHTML = "<strong>Ready?</strong><div class='bi-muted'>Tracking starts on the first active block in round 1.</div>"; const start = document.createElement("button"); start.className = "btn btn-primary"; start.textContent = "Start encounter"; start.onclick = () => void startEncounter(request, preview); action.append(actionText, start); section.append(action); }
    results.append(section);
}

function renderTie(preview: InitiativePreviewResponse, request: InitiativePreviewRequest): HTMLElement {
    const box = document.createElement("section"); box.className = "bi-message bi-warning"; const title = document.createElement("strong"); title.textContent = "DM ruling needed"; const description = document.createElement("p"); description.textContent = "Opposing placements tied. Reorder the tied units only; tactical groups stay together."; box.append(title, description);
...
function roundBoundary(preview: InitiativePreviewResponse): HTMLElement {
    ...
    else { const bottom = preview.blocks.findIndex(block => block.id === preview.cyclicMerge!.bottomBlockId) + 1; const top = preview.blocks.findIndex(block => block.id === preview.cyclicMerge!.topBlockId) + 1; note.textContent = `Round 1: Block ${bottom} skips its separate activation. Round 2 onward: it joins Block ${top} across the round boundary.`; }
...
async function renderRunner(request: InitiativePreviewRequest, preview: InitiativePreviewResponse, advanceCount: number): Promise<void> { if (!stateUrl) return; results.innerHTML = '<section class="card card-body">Loading encounter…</section>'; try { const state = await loadInitiativeTurnState(stateUrl, { ...request, advanceCount }); renderRunnerState(request, preview, state, advanceCount); } catch (error) { setup.hidden = false; results.replaceChildren(); showError(error); } }
...
function renderRunnerState(request: InitiativePreviewRequest, preview: InitiativePreviewResponse, state: InitiativeTurnStateResponse, advanceCount: number): void {
    ...
    if (active) { const activePanel = document.createElement("div"); activePanel.className = "bi-active bi-grid"; const head = document.createElement("div"); head.className = "bi-row"; const text = document.createElement("div"); const activeHeading = document.createElement("h4"); activeHeading.className = "h5 mb-0"; activeHeading.textContent = `${friendly(active.allianceId)} block`; const hint = document.createElement("div"); hint.className = "bi-muted"; hint.textContent = "Finish this block before advancing."; text.append(activeHeading, hint); head.append(text, blockBadges(active.allianceId, active.blockType)); activePanel.append(head); const members = document.createElement("div"); members.className = "bi-list"; for (const id of active.memberOrder) { const combatant = byId.get(id); const member = document.createElement("div"); member.className = "bi-runner-member bi-row"; const name = document.createElement("strong"); name.textContent = combatant?.name ?? id; const meta = document.createElement("span"); meta.className = "bi-muted"; meta.textContent = combatant?.tacticalGroupId ? groupName(combatant.tacticalGroupId) : ""; member.append(name, meta); members.append(member); } activePanel.append(members); if (active.allianceId === "players" && active.memberOrder.length > 1) { const note = document.createElement("div"); note.className = "bi-muted"; note.textContent = "Players may act in any order within this block."; activePanel.append(note); } card.append(activePanel); }
    const sequence = document.createElement("div"); sequence.className = "bi-sequence"; state.blocks.forEach((block, index) => { const chip = document.createElement("span"); chip.className = `bi-seq${block.id === state.activeBlockId ? " active" : ""}`; chip.textContent = `${index + 1}. ${friendly(block.allianceId)}${block.blockType === "kaiju" ? " · Kaiju" : ""}`; sequence.append(chip); }); card.append(sequence);
    const actions = document.createElement("div"); actions.className = "bi-actions bi-primary"; const next = document.createElement("button"); next.className = "btn btn-primary"; next.textContent = "Next block"; next.onclick = () => void renderRunner(request, preview, advanceCount + 1); actions.append(next); card.append(actions); results.append(card);
}

function groupName(groupId: string): string { const group = Array.from(enemyGroups.querySelectorAll<HTMLElement>(".bi-tactical-group")).find(candidate => candidate.dataset.groupId === groupId); return group ? query<HTMLInputElement>("[data-role='group-name']", group).value.trim() || "Tactical group" : "Tactical group"; }
...
function blockBadges(allianceId: string, blockType: TurnBlockType): HTMLElement { const container = document.createElement("div"); container.className = "bi-badges"; container.append(badge(friendly(allianceId))); if (blockType === "kaiju") container.append(badge("Kaiju", true)); return container; }
