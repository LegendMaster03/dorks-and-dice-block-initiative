...
export function projectMonsterCombatStats(
    ...
    const abilitySaves = readAbilitySaves(document);
    const useFifthEditionSaveDefaults = isFifthEdition(editionDisplayName);
    ...
    for (const key of abilityKeys) {
        ...
        const explicitSave = abilitySaves.get(property) ?? null;
        abilities[key] = {
            score,
            modifier,
            save: explicitSave ?? (useFifthEditionSaveDefaults ? modifier : null)
        };
    }
...
function readAbilitySaves(document: Record<string, unknown>): Map<string, number> {
    ...
    for (const source of [document.save, document.savingThrows]) {
        ...
        for (const key of abilityKeys) {
            ...
            if (value !== null) saves.set(property, value);
        }
    }
...
function readArmorClass(value: unknown): string | null {
    if (typeof value === "number" || typeof value === "string") {
        return leadingNumber(String(value)) ?? String(value);
    }
    if (!Array.isArray(value) || value.length === 0) return null;
    const first = value[0];
    if (typeof first === "number" || typeof first === "string") return leadingNumber(String(first)) ?? String(first);
    if (isRecord(first)) {
        const ac = first.ac;
        if (typeof ac === "number" || typeof ac === "string") return leadingNumber(String(ac)) ?? String(ac);
    }
    return null;
}
...
function formatDefense(value: unknown): string | null {
    ...
    const nestedKeys = ["resist", "immune", "vulnerable", "conditionImmune", "damage", "types"];
    const nested = nestedKeys.map(key => formatDefense(value[key])).filter((part): part is string => Boolean(part));
...
function legacyFieldMap(value: unknown): Map<string, string> {
    ...
    const labels = [
        ...
        "Armor Class", "Hit Points", "Hit Dice", "Initiative", "Speed", "AC", "Init",
...
function hitPointsFromHitDice(value: unknown): number | null {
    ...
    const match = text.match(/\((\d+)\s*hp\)/i);
...
function isFifthEdition(value: string): boolean {
    return /(?:\b5(?:\.5)?e\b|\b5th\b|\bfifth\b|\b2014\b|\b2024\b)/i.test(value);
}
